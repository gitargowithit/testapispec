...
import { testapispecipluginPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [testapispecipluginPlugin],
    });

    await request(server).get('/api/testapispeciplugin/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/testapispeciplugin/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/testapispeciplugin/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/testapispeciplugin/todos/${createdTodoItem.id}`)
